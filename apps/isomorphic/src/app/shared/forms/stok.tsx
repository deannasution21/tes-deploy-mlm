...
import { StokFormInput, stokFormSchema } from '@/validators/stok-form-schema';
...
function Formnya({
  ...
  `}
            ...
            <Textarea
              ...
              value={product?.attribute?.description}
              disabled
            />
            <Input
              label="Harga"
              prefix={'Rp'}
              type="number"
              value={product?.attribute?.price?.amount}
              disabled
            />
            ...
            <Controller
              ...
              render={({ field: { name, onChange, value } }) => (
                <QuantityInput
                  label="Min Order"
                  name={name}
                  value={product?.attribute?.min_order_quantity}
                  onChange={onChange}
                  disabled
                />
              )}
      ...
      <div className="-mb-4 flex items-center justify-end gap-4 border-t py-4 dark:bg-gray-50">
        <Button
          ...
          onClick={() => router.push(routes.stok.manajemen.index)}
        >
          Batal
        </Button>
        <Button
...
export default function StokFormPage({
  type,
  product_id,
}: {
  type?: string;
  ...
  const doSave = async (payload: any, reset: any) => {
    ...
    const payloadnya =
      type === 'tambah' ? payload : { ...payload, type: 'stock' };
    ...
    fetchWithAuth<any>(
      ...
      .then((data) => {
        ...
        if (type === 'tambah') {
          ...
          router.push(routes.stok.manajemen.index);
        }
      })
      .catch((error) => {
        console.error(error);
  ...
  const onSubmit: SubmitHandler<StokFormInput> = (data) => {
    setLoadingS(true);
  ...
  return (
    <div className="@container">
      <div className="grid grid-cols-1 gap-6 3xl:gap-8">
        <WidgetCard
          title={<span className="text-[#c69731]">Form {typenya} Stok</span>}
