...
import {
  PembagianBonusReport,
  PembagianBonusReportAnalytics,
  PembagianBonusReportDetail,
} from '@/types/report-pembagian-bonus';

export default function ReportPembagianGajiPage({
  className,
}: {
  className?: string;
}) {
  ...
  const [dataAnalytics, setDataAnalytics] =
    useState<PembagianBonusReportAnalytics | null>(null);
  ...
  useEffect(() => {
    ...
    fetchWithAuth<PembagianBonusReport>(
      `/_reports?type=bonus_salary&period=${type}${type === 'monthly' ? `&year_month=${formatToYearMonth(startDate)}` : ''}${type === 'custom' ? `&start_date=${formatToYMD(custom[0])}&end_date=${formatToYMD(custom[1])}` : ''}&report_type=details`,
      { method: 'GET' },
      session.accessToken
    )
      .then((data) => {
