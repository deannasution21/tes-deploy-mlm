...
export default function ReportPembayaranGajiPage({
  className,
}: {
  className?: string;
}) {
  ...
  useEffect(() => {
    ...
    fetchWithAuth<WithdrawalBonusReport>(
      `/_reports?type=withdrawal_salary&period=${type}${type === 'monthly' ? `&year_month=${formatToYearMonth(startDate)}` : ''}${type === 'custom' ? `&start_date=${formatToYMD(custom[0])}&end_date=${formatToYMD(custom[1])}` : ''}&report_type=details`,
      { method: 'GET' },
      session.accessToken
    )
      .then((data) => {
