...
export default function HistoryTransferPointWisataTable({
  className,
  typenya,
}: {
  className?: string;
  ...
  const [type, setType] = useState<string>(typenya ?? 'send');
  ...
  useEffect(() => {
    ...
    fetchWithAuth<HistoryTransferPinResponse>(
      `/_point-promo-wisata/history?type=${type}`,
      { method: 'GET' },
      session.accessToken
    )
      .then((data) => {
