...
import { topIncomeColumns } from './columns';
...
import { LeaderboardItem, LeaderboardResponse } from '@/types/top-income';

export default function TopIncomeTable({ className }: { className?: string }) {
  const { data: session } = useSession();
  const [isLoading, setLoading] = useState(true);

  const [dataIncome, setDataIncome] = useState<LeaderboardItem[]>([]);
  ...
  useEffect(() => {
    ...
    fetchWithAuth<LeaderboardResponse>(
      `/_reports/documents?type=leaderboard`,
      { method: 'GET' },
      session.accessToken
    )
      .then((data) => {
        const datanya = data?.data?.leaderboards || [];
        setDataIncome(datanya);
        setData(datanya);
      })
      .catch((error) => {
        console.error(error);
  ...
  const { table, setData } = useTanStackTable<LeaderboardItem>({
    tableData: dataIncome,
    columnConfig: topIncomeColumns,
    options: {
      initialState: {
        pagination: {
          pageIndex: 0,
