    ...
    if (!res.ok) {
      ...
      const extractErrorMessage = (
        ...
        if (Array.isArray(errorField)) {
          // Join array messages with comma separator
      ...
      const message =
        extractErrorMessage(errBody?.message) ||
        extractErrorMessage(errBody?.error) ||
...
export async function fetchWithAuthBU<T = any>(
  endpoint: string,
  options: RequestInit = {},
  token?: string
): Promise<T> {
